...
class DetailViewControllerTests: XCTestCase {
    ...
    func setupTaskAndAppearanceTransition() {
        let coordinate = CLLocationCoordinate2D(latitude: 37.768374, longitude: -122.51180649)
        let location = Location(name: "Baz", coordinate: coordinate)
        let date = Date(timeIntervalSince1970: 1608681600)
    ...
    func testSettingTaskSetsDateLabel() {
        setupTaskAndAppearanceTransition()
        XCTAssertEqual(sut.dateLabel.text, "23.12.20")
    }
    
    func testSettingTaskSetsMapView() {
        setupTaskAndAppearanceTransition()
        XCTAssertEqual(sut.mapView.centerCoordinate.latitude, 37.768374, accuracy: 0.001)
        XCTAssertEqual(sut.mapView.centerCoordinate.longitude, -122.51180649, accuracy: 0.001)
    }
