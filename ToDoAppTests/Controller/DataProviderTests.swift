...
class DataProviderTests: XCTestCase {
    ...
    func testDeleteButtonTitleSectionZeroShowsDone(){
        let buttonTitle = tableView.delegate?.tableView?(tableView,
    ...
    func testUnchekingTaskUnchecksInTaskManager(){
        let task = Task(title: "Foo")
        sut.taskManager?.add(task: task)
        sut.taskManager?.checkTask(at: 0)
        tableView.reloadData()
...
extension DataProviderTests {
    class MockTableView: UITableView {
        ...
        static func mockTableView(withDataSource dataSource: UITableViewDataSource) -> MockTableView {
            let mockTableView = MockTableView(frame: CGRect(x: 0, y: 0, width: 390, height: 844)) //по умолчанию размер mockTableView 0,0
